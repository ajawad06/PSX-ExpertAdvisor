...
function setupEventListeners() {
  ...
  document
    .getElementById("generatePortfolioBtn")
...
async function loadDashboard() {
  ...
  try {
    ...
    if (scanData.success) {
      const buySignals = scanData.results.filter(
        (r) => r.signal === "BUY"
      ).length;
      const sellSignals = scanData.results.filter(
        (r) => r.signal === "SELL"
      ).length;
      const holdSignals = scanData.results.filter(
        (r) => r.signal === "HOLD"
      ).length;
    ...
    hideLoading("marketOverviewLoading");
    showElement("marketOverviewContent");
    document.getElementById("topMoversContainer").innerHTML =
      '<p style="color: var(--text-secondary); text-align: center;">Failed to load market data</p>';
  }
}
  ...
  top5.forEach((stock) => {
    ...
    card.innerHTML = `
            <div class="allocation-header">
                ...
                <span class="signal-badge signal-badge-${stock.signal.toLowerCase()}">${
      stock.signal
    }</span>
            </div>
            <div class="allocation-details">
                <div class="allocation-detail">
                    ...
                    <span class="allocation-detail-value">PKR ${stock.price.toFixed(
                      2
                    )}</span>
                </div>
                <div class="allocation-detail">
                    <span class="allocation-detail-label">Change</span>
                    <span class="allocation-detail-value" style="color: ${
                ...
                <div class="allocation-detail">
                    ...
                    <span class="allocation-detail-value">${
                      stock.confidence
                    }%</span>
                </div>
            </div>
...
function displayAnalysis(data) {
  ...
  signalDiv.textContent = `${
    decision === "BUY" ? "🟢" : decision === "SELL" ? "🔴" : "⏸️"
  } STRONG ${decision} SIGNAL`;

  // Metrics
  document.getElementById(
    "metricPrice"
  ).textContent = `PKR ${data.price.current.toFixed(2)}`;

  const changeSpan = document.getElementById("metricChange");
  changeSpan.textContent = `${
    data.price.change >= 0 ? "+" : ""
  }${data.price.change.toFixed(2)} (${
    data.price.change_percent >= 0 ? "+" : ""
  }${data.price.change_percent.toFixed(2)}%)`;
  changeSpan.className = `metric-change ${
    data.price.change >= 0 ? "positive" : "negative"
  }`;

  document.getElementById(
    "metricConfidence"
  ).textContent = `${data.analysis.confidence}%`;
  ...
  document.getElementById("metricVolume").textContent =
    new Intl.NumberFormat().format(data.price.volume);
...
function displayStockNews(ticker, newsData) {
  ...
  if (newsData && newsData.length > 0) {
    ...
    const validNews = newsData.filter((item) => {
      ...
      if (
        title.includes("ispermalink") ||
        title.includes("<item>") ||
        title.includes("xml file") ||
        title.includes("this feed") ||
        summary.includes("ispermalink") ||
      ...
      validNews.forEach((item, index) => {
        ...
        newsItem.innerHTML = `
                   ...
                   <div class="news-published">📅 ${item.published}</div>
                    <div class="news-summary">${cleanSummary.substring(
                      0,
                      300
                    )}${cleanSummary.length > 300 ? "..." : ""}</div>
      ...
      datasets: [
        ...
        },
        {
          ...
          backgroundColor: hist.map((v) =>
            v >= 0 ? "rgba(0, 200, 83, 0.5)" : "rgba(255, 23, 68, 0.5)"
          ),
        },
      ],
    },
...
function displayPortfolio(data) {
  ...
  document.getElementById(
    "summaryBudget"
  ).textContent = `PKR ${new Intl.NumberFormat().format(
    data.summary.total_budget
  )}`;
  document.getElementById(
    "summaryInvested"
  ).textContent = `PKR ${new Intl.NumberFormat().format(
    Math.round(data.summary.total_invested)
  )}`;
  document.getElementById(
    "summaryRemaining"
  ).textContent = `PKR ${new Intl.NumberFormat().format(
    Math.round(data.summary.cash_remaining)
  )}`;
  document.getElementById("summaryStocks").textContent =
    data.summary.num_stocks;
    ...
    card.innerHTML = `
            <div class="allocation-header">
                ...
                <div style="display: flex; gap: 0.5rem; align-items: center;">
                    <span class="allocation-signal">BUY ${
                      stock.confidence
                    }%</span>
                </div>
            </div>
            <div class="allocation-details">
                ...
                <div class="allocation-detail">
                    ...
                    <span class="allocation-detail-value">PKR ${new Intl.NumberFormat().format(
                      Math.round(stock.investment)
                    )}</span>
                </div>
                <div class="allocation-detail">
                    <span class="allocation-detail-label">Portfolio %</span>
                    <span class="allocation-detail-value">${stock.allocation_percent.toFixed(
                      1
                    )}%</span>
                </div>
                <div class="allocation-detail">
                    <span class="allocation-detail-label">RSI</span>
                    <span class="allocation-detail-value">${stock.rsi.toFixed(
                      2
                    )}</span>
                </div>
            </div>
...
function createScannerCard(stock) {
  ...
  card.innerHTML = `
        ...
        <div class="scanner-details">
            <span class="${changeClass}">${changeSign}${stock.change_percent.toFixed(
    2
  )}%</span>
